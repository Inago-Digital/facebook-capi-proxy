...
function validateEventSourceUrls(events, expectedOrigin) {
  for (const event of events) {
    const sourceUrl = typeof event?.event_source_url === 'string' ? event.event_source_url.trim() : '';

    if (!sourceUrl) {
      if (EVENT_REQUIRE_EVENT_SOURCE_URL) {
        return { ok: false, status: 400, error: 'event_source_url is required for each event' };
      }
      continue;
    }
    ...
    if (!sourceOrigin) {
      return { ok: false, status: 400, error: 'event_source_url must be a valid URL' };
    }

    if (sourceOrigin !== expectedOrigin) {
      return { ok: false, status: 403, error: 'event_source_url origin mismatch' };
    }
  }
...
function consumeSiteIpRateLimit(siteId, ip) {
  ...
  if (!current || current.resetAt <= now) {
    siteIpRateBuckets.set(key, { count: 1, resetAt: now + EVENT_SITE_RATE_LIMIT_WINDOW_MS });
...
async function issueEventAccessToken(siteId, origin, ip) {
  ...
  await db.run(
    `INSERT INTO event_tokens (id, site_id, token_hash, origin, ip, expires_at)
     VALUES (?, ?, ?, ?, ?, ?)`,
    [crypto.randomUUID(), siteId, hashToken(token), origin, ip || null, expiresAt]
  );
...
async function requireEventAccessToken(req, res, next) {
  ...
  try {
    ...
    if (req.requestOrigin && record.origin && req.requestOrigin !== record.origin) {
      return res.status(401).json({ error: 'Unauthorized' });
    }
...
router.post('/auth', requireSite, async (req, res) => {
  ...
  const sourceUrl = typeof req.body?.event_source_url === 'string' ? req.body.event_source_url.trim() : '';
  if (sourceUrl) {
    const sourceOrigin = normalizeOrigin(sourceUrl);
    if (!sourceOrigin) {
      return res.status(400).json({ error: 'event_source_url must be a valid URL' });
    }
...
router.post('/', requireSite, requireEventAccessToken, async (req, res) => {
  ...
  for (const event of body.data) {
    ...
    if (!eventId) {
      return res.status(400).json({ error: 'event_id is required for each event' });
    }
    if (payloadSeen.has(eventId)) {
      return res.status(409).json({ error: 'Duplicate event_id in payload', duplicate_event_id: eventId });
    }
  ...
  const enrichedData = body.data.map((event) => {
    ...
    return {
      ...
      user_data: {
        ...
        client_user_agent: ud.client_user_agent || req.headers['user-agent'] || '',
        ...ud,
      },
    };
  });
  ...
  try {
    ...
    return res.status(502).json({ error: 'Failed to reach Facebook CAPI', details: err.message });
  } finally {
