...
- [ ] Add root `pnpm` scripts to run both `server` and `dashboard` together (`dev`, `build`) for easier local workflow
...
- [ ] Add a short "Production checklist" section in both READMEs (CORS origins, HTTPS, token TTL, backups)
